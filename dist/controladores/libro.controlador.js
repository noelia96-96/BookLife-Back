...
class libroController {
    ...
    reservarLibro(req, res) {
        ...
        usuario_modelo_1.Usuario.findById(_id).then((usuarioDB) => {
            if (!usuarioDB) {
                ...
                libro_modelo_1.Libro.findOne({ nombreLibro: params.nombreLibro }).then(libroDB => {
                    if (!libroDB) {
                        return res.status(400).send({
                            status: 'error',
                            mensaje: 'El libro no existe',
    ...
    mostrarLibrosPicharCard(req, res) {
        console.log(req);
        let _id = req.body.usuario._id;
        let params = req.body;
        ...
        usuario_modelo_1.Usuario.findById(_id).then((usuarioDB) => {
            if (!usuarioDB) {
                ...
                libro_modelo_1.Libro.find({ creador: libreriaPinchada }).then((librosDB) => {
                    if (!librosDB) {
                        return res.status(200).send({
                            status: 'error',
